...
- Corpus audit EP9M context rows in master: **153**.
...
- All-row coverage minimum, legacy rows included: **20.9%**
- Source-capable field statuses:
| field                   |   coverage_pct | status   |
|:------------------------|---------------:|:---------|
| day1_close_pct_in_range |           98.8 | OK       |
| day1_move_pct           |           98.8 | OK       |
...
- Rows: **191**; full-plan proxy rows: **5**; rows with ≥5 future bars: **83**; OK-evaluable rows: **5**.
- DECISION_LOG rows target/R:R enriched inside learning loop from universe planning fields: **0**
- Entry-source classification (v4.13.73): **CAPTURED_AT_TRIGGER=11** (verdict-eligible pool, gated on T+5 maturity); BACKFILLED_FROM_SIGNAL_CLOSE=0 (robustness panel only, excluded from verdict); MISSING_ENTRY_UNRECOVERABLE=180 (excluded).
- Latest pack health: `2026-06-01` **6/108** rows CAPTURED_AT_TRIGGER (partial capture, investigate).
...
| P1         | Confirm zero-TRADE root cause                | Post-2026-05-15 rows have zero TRADE rows (n=84). Confirm whether this is intended strictness or over-routing to COUNCIL/WATCH.                                              |
...
Valid longer-lookback thrust rows: 9
...
- valid_longer_lookback_thrust_flag: 18
...
|---|---|---|---|
| WSR | 5 | 2026-05-05 | 2026-06-01 |
| SILA | 4 | 2026-05-05 | 2026-06-01 |
...
| 1 | RAMP | 2026-06-01 | DATA_ISSUE | Extreme close-to-close move detected: max_abs_c2c_80d=27.3%. Possible corporate action, split-adjustment issue, merger, or bad price data. |
...
- Join failures (NO_MATCH): 795
- Rows missing trigger_price: 695
- Rows missing invalidation_price: 695
